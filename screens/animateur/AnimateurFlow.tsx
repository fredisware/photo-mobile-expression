...
import { Play, Pause, FastForward, CheckCircle, Users, Clock, Image as ImageIcon, MessageCircle, FileText, UserPlus, X, ChevronLeft, ChevronRight, Smartphone, EyeOff, Scale, MicOff, Lock, Trash2, Shuffle, Plus, Download, Repeat, ArrowRight } from 'lucide-react';
...
const PrintableReport = ({ session }: { session: any }) => {
    const portalDiv = document.getElementById('print-portal');
    if (!portalDiv) return null;

    const content = (
        <div id="print-container" style={{display: 'none', padding: '40px', fontFamily: 'Arial, sans-serif'}}>
            <h1 style={{fontSize: '24px', fontWeight: 'bold', color: '#1C1C1E', marginBottom: '10px'}}>{session.theme}</h1>
            <p style={{fontSize: '14px', color: '#777', marginBottom: '30px'}}>Question: {session.taskQuestion}</p>
            
            <div style={{marginBottom: '40px'}}>
                <h2 style={{fontSize: '18px', borderBottom: '2px solid #eee', paddingBottom: '10px', marginBottom: '15px'}}>Notes de l'animateur</h2>
                <div style={{whiteSpace: 'pre-wrap', lineHeight: '1.6', fontSize: '12px'}}>
                    {session.notes || "Pas de notes."}
                </div>
            </div>

            <div>
                <h2 style={{fontSize: '18px', borderBottom: '2px solid #eee', paddingBottom: '10px', marginBottom: '20px'}}>Photos choisies</h2>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px'}}>
                    {session.participants.filter((p:any) => p.selectedPhotoId).map((p:any) => {
                        const pic = session.photos.find((ph:any) => ph.id === p.selectedPhotoId);
                        return (
                            <div key={p.id} style={{pageBreakInside: 'avoid'}}>
                                <img 
                                    src={pic?.url} 
                                    style={{width: '100%', height: '200px', objectFit: 'cover', borderRadius: '8px', marginBottom: '8px'}} 
                                />
                                <p style={{fontWeight: 'bold', fontSize: '12px'}}>{p.name}</p>
                            </div>
                        )
                    })}
                </div>
            </div>
            <div style={{marginTop: '50px', fontSize: '10px', color: '#999', textAlign: 'center'}}>
                Généré par Photo-mobile-expression - {new Date().toLocaleDateString()}
            </div>
        </div>
    );
...
const AnimateurFlow = () => {
  const { 
    session, startSession, startSilentPhase, toggleTimer, startSelectionPhase, 
    startSpeakingTour, startDebateTour, nextSpeaker, endSession, resetSession, updateNotes, addGuestParticipant, 
    selectPhoto, removeParticipant, addTime, forceRandomSelection 
  } = useSession();
  ...
  if (session.stage === SessionStage.LOBBY) {
      const isAnimateurParticipating = session.participants.some(p => p.name === "Animateur");
      ...
      return (
          <div className="flex flex-col h-full px-5 pt-6 pb-6">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar">
                ...
                <GuestAddForm onAdd={addGuestParticipant} />
                
                {!isAnimateurParticipating && (
                    <div className="flex justify-center mb-6">
                        <button 
                            onClick={() => addGuestParticipant("Animateur")}
                    ...
                    {session.participants.map(p => (
                        <div key={p.id} className="bg-white p-3 rounded-2xl flex items-center gap-3 shadow-sm group">
                            ...
                            <button 
                                type="button"
                                onClick={() => {
                                    if(window.confirm(`Exclure ${p.name} de la séance ?`)) {
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar">
                  ...
                  <div className="space-y-4 px-2 pb-4">
                    <div className="flex gap-4">
                        ...
                        <p className="text-sm text-gray-600">Il n'y a pas de bonne ou de mauvaise réponse.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                            <Scale size={16} />
                        </div>
                        <p className="text-sm text-gray-600">Il n'y a pas de jugement.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-purple-50 text-purple-600 flex items-center justify-center shrink-0">
                            <Lock size={16} />
                        </div>
                        <p className="text-sm text-gray-600">Ce qui se dit dans l'atelier reste dans l'atelier.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-orange-50 text-orange-600 flex items-center justify-center shrink-0">
                            <Users size={16} />
                        </div>
                        <p className="text-sm text-gray-600">La participation n'est pas facultative.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center shrink-0">
                            <EyeOff size={16} />
                        </div>
                        <p className="text-sm text-gray-600">Il n'y a pas d'obligation d'assiduité.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0">
                            <MessageCircle size={16} />
                        </div>
                        <p className="text-sm text-gray-600">Nous gardons une attitude respectueuse.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="w-8 h-8 rounded-full bg-red-50 text-red-600 flex items-center justify-center shrink-0">
                            <Smartphone size={16} />
                        </div>
                        <p className="text-sm text-gray-600">Nous mettons nos portables en silencieux uniquement.</p>
                    </div>
                  </div>
              </div>
      ...
      return (
          <div className="flex flex-col h-full px-5 pb-6">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar">
                 ...
                 <div className="grid grid-cols-1 gap-3 pb-4">
                    {session.participants.map(p => (
                        <div key={p.id} className="bg-white p-3 rounded-2xl flex items-center gap-3 shadow-sm border border-transparent transition-colors group">
              ...
              {allSelected ? (
                   ...
                   <Button 
                    variant="secondary" 
                    onClick={() => {
                        if(window.confirm("Cela va attribuer des photos aléatoires aux retardataires et démarrer le tour de parole immédiatement. Continuer ?")) {
                      ...
                      <div className="relative rounded-[24px] overflow-hidden shadow-md aspect-square bg-gray-100">
                          ...
                          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/60 to-transparent p-6 pt-12">
                              <div className="flex items-center gap-3 text-white">
                                  ...
                                  {speaker.isGuest && <span className="text-xs bg-white/20 px-2 py-0.5 rounded-full ml-auto">Invité</span>}
                              </div>
                          </div>
                      </div>
  ...
  if (session.stage === SessionStage.ROUND_TRANSITION) {
      return (
          <div className="flex flex-col h-full px-5 pb-6 pt-10 text-center">
  ...
  if (session.stage === SessionStage.SYNTHESIS || session.stage === SessionStage.ENDED) {
      const handleExportPDF = () => {
          setIsExporting(true);
          // Allow render cycle to update before printing
          setTimeout(() => {
              window.print();
                ...
                <div className="flex flex-col gap-2 mt-4">
                    <div className="flex gap-2">
                        ...
                        <Button 
                            ...
                            fullWidth 
                            onClick={() => {
                                if(window.confirm("Voulez-vous vraiment quitter et réinitialiser la séance ?")) {
                                    resetSession();
                                }
                            }} 
                            variant="ghost" 
